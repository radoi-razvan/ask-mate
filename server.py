...
def search():
    ...
    for question_id in question_id_list:
        questions_data.append(
            data_handler.get_all_data_for_id(ct.TABLE_QUESTION, question_id)[0]
        )
    for answer_id in answer_id_list:
        answers_data.append(
            data_handler.get_all_data_for_id(ct.TABLE_ANSWER, answer_id)[0]
        )
...
def route_list():
    ...
    if "username" in session:
        ...
        user_id = None
    return render_template(
        "list.html", questions=questions, username=username, user_id=user_id
    )
...
def route_question(question_id):
    ...
    tag_data = []
    tag_ids_list = data_handler.get_all_data_for_id(
        ct.TABLE_QUESTION_TAG, question_id, "question_id"
    )
    ...
    if tag_ids_list:
        for element in tag_ids_list:
            ...
            data.update(
                {
                    "tag_name": data_handler.get_data_for_id(
                        ct.TABLE_TAG, element["tag_id"], "name"
                    )
                }
            )
            tag_data.append(data)
    print("comm data", all_comments_data[2]["user_id"])
    accepted_answer_id = data_handler.get_data_for_id(
        ct.TABLE_QUESTION, question_id, "accepted_answer_id", el_id="id"
    )
    return render_template(
        "question.html",
...
def edit_question_route(question_id):
    if "username" in session:
        ...
        message = data_handler.get_data_for_id(
            ct.TABLE_QUESTION, question_id, "message"
        )
        return render_template(
            "edit_question.html",
...
def vote_up_question_route(question_id):
    if "username" in session:
        username = session["username"]
        vote_type = 1
        user_id = data_handler.get_data_for_id(
            ct.TABLE_QUESTION, question_id, "user_id", el_id="id"
        )
...
def edit_comment_route(comment_id):
    if "username" in session:
        ...
        question_id = data_handler.get_option_id_with_comment_id(
            comment_id, requested_id
        )
        if question_id is None:
            requested_id = "answer_id"
            answer_id = data_handler.get_option_id_with_comment_id(
                comment_id, requested_id
            )
...
def route_tags():
    ...
    if "username" in session:
        ...
        user_id = session["user_id"]
        return render_template(
            "tags_page.html", tags_data=tags_data, username=username, user_id=user_id
        )
...
def login():
    if request.method == "POST":
        ...
        try:
            if username == data_handler.get_user_column(username, "name")[0][
                "name"
            ] and cy.verify_password(
                password,
                data_handler.get_user_column(username, "password")[0]["password"],
            ):
                session.update({"username": username})
                session.update(
                    {
                        "user_id": data_handler.get_user_column(
                            session["username"], "id"
                        )[0]["id"]
                    }
                )
...
def registration_route():
    if request.method == "POST":
        ...
        password = request.form["password"]
        if (
            not data_handler.get_user_column(user_name, "name")
            and len(user_name) > 5
            and len(password) > 5
        ):
...
def users():
    if "username" in session:
        ...
        users_data = data_handler.get_data_unsorted(ct.TABLE_USERS, "registration_date")
        return render_template(
            "users.html", users_data=users_data, username=username, user_id=user_id
        )
    return redirect(url_for("route_home"))
...
def user_profile(user_id):
    if "username" in session:
        username = session["username"]
        user_data = data_handler.get_all_data_for_id(
            ct.TABLE_USERS, user_id, el_id="id"
        )[0]
        question_data = data_handler.get_all_data_for_id(
            ct.TABLE_QUESTION, user_id, el_id="user_id"
        )
        answer_data = data_handler.get_all_data_for_id(
            ct.TABLE_ANSWER, user_id, el_id="user_id"
        )
        comment_data = data_handler.get_all_data_for_id(
            ct.TABLE_COMMENT, user_id, el_id="user_id"
        )
        comments = []
        for comment in comment_data:
            if comment["question_id"] is None:
                question_id = data_handler.get_question_id_with_answer_id(
                    comment["answer_id"]
                )
...
def accepted(question_id, answer_id):
    if "username" in session:
        ...
        try:
            ...
            form_value = None
        user_id = data_handler.get_data_for_id(
            ct.TABLE_ANSWER, answer_id, "user_id", el_id="id"
        )
